...
export const LoyaltyPlansPage: React.FC = () => {
  const { campaigns, deleteCampaign, addToast } = useStore();
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');

  // Filter campaigns of category 'loyalty_plan'
  const plans = campaigns.filter(
    (c) =>
      c.campaignCategory === 'loyalty_plan' &&
      c.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const handleDelete = async (id: string, name: string) => {
    if (window.confirm(`Are you sure you want to delete Loyalty Plan "${name}"?`)) {
      try {
        await deleteCampaign(id);
        addToast(`Loyalty Plan "${name}" deleted successfully`, 'success');
      } catch (err) {
      ...
      <main className="flex-grow p-4 md:p-6 overflow-y-auto bg-slate-50 h-[calc(100vh-60px)]">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-6">
          <div>
            ...
            <p className="text-xs text-gray-500">Loyalty Plans</p>
          </div>
          <div>
            <Link
              to="/loyalty/plans/new"
        ...
        {plans.length > 0 ? (
                ...
                <tbody className="divide-y divide-gray-100 text-sm text-gray-850 font-medium">
                  {plans.map((plan) => (
                    <tr key={plan.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-500">{plan.startDate}</td>
                      <td className="px-6 py-4 text-gray-500">{plan.endDate}</td>
                      <td className="px-6 py-4">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
                            plan.isActive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                          }`}
                        >
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-12 shadow-sm flex flex-col items-center justify-center text-center mt-6 min-h-[350px]">
            <div className="relative mb-6">
              ...
              >
                <path
                  d="M125 15C136 25 136 45 130 55C124 65 110 70 100 68"
                  stroke="#e2e8f0"
                  strokeWidth="2"
                  strokeDasharray="4 4"
                />
                <path
                  d="M30 45C22 52 18 68 25 78C32 88 50 82 58 75"
                  stroke="#e2e8f0"
                  strokeWidth="2"
                  strokeDasharray="4 4"
                />
