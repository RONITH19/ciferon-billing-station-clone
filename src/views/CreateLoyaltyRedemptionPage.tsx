...
export const CreateLoyaltyRedemptionPage: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      addToast('Name of Reward is required', 'warning');
      return;
    }

    try {
      await addCampaign({
        campaignCategory: 'redemption',
        name: name.trim(),
        description: description.trim(),
        promocode: '',
        type: 'Redemption Reward',
        startDate,
        endDate,
        isActive,
        startingFrom,
      ...
      addToast(`Redemption Reward "${name}" created successfully`, 'success');
      navigate('/loyalty/redemption');
    } catch (err) {
      addToast(err instanceof Error ? err.message : 'Failed to create redemption', 'error');
    }
  };
      ...
      <main className="flex-grow p-4 md:p-6 overflow-y-auto bg-slate-50 h-[calc(100vh-60px)]">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-6">
          <div>
            ...
            <p className="text-xs text-gray-500">Add New Redemption</p>
          </div>
          <div className="flex items-center gap-3">
            <Link
              to="/loyalty/redemption"
        ...
        <form onSubmit={handleSave} className="max-w-7xl flex flex-col gap-6">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
            ...
            <div className="lg:col-span-3 flex flex-col gap-6">
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-5 flex flex-col gap-4 shadow-sm">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-600 mb-1.5">
                      Name of Reward<span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={name}
              ...
              {/* Conditions Card matching Image 7 */}
              <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm flex flex-col gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1.5">
                    Redemption Condition / Criteria
                  </label>
                  <input
                    type="text"
                    value={conditions}
                    onChange={(e) => setConditions(e.target.value)}
                    placeholder="e.g. Min 500 points needed to redeem"
                    className="w-full px-3 py-2 border border-gray-250 rounded-lg text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/10 transition-all font-medium text-gray-800"
                  />
                </div>
              </div>
